...
vi.mock('../src/util/transform', () => ({
  ...
  transform: vi.fn().mockImplementation(async function(code, input, context, _skipWrap, _inputType) {
...
vi.mock('../src/util/fileReference', async () => ({
  ...(await vi.importActual('../src/util/fileReference')),

  processConfigFileReferences: vi.fn().mockImplementation(async function(config) {
    if (
...
vi.mock('glob', () => ({
  globSync: vi.fn().mockImplementation(function(pattern) {
...
vi.mock('../src/evaluatorHelpers', async () => ({
  ...
  runExtensionHook: vi.fn().mockImplementation(function(_extensions, _hookName, context) {
    return context;
  })
}));
...
vi.mock('../src/util/fileExtensions', () => ({
  isImageFile: vi
    .fn()
    .mockImplementation(function(filePath) {
    return filePath.endsWith('.jpg') || filePath.endsWith('.png');
  }),
  isVideoFile: vi.fn().mockImplementation(function(filePath) {
    return filePath.endsWith('.mp4');
  }),
  isAudioFile: vi.fn().mockImplementation(function(filePath) {
...
vi.mock('../src/util/functions/loadFunction', async () => ({
  ...(await vi.importActual('../src/util/functions/loadFunction')),

  loadFunction: vi.fn().mockImplementation(function(options) {
  ...
  parseFileUrl: (await vi.importActual('../src/util/functions/loadFunction')).parseFileUrl
}));
...
describe('evaluator', () => {
  ...
  it('runs Simba test cases after other providers during evaluate', async () => {
    ...
    const agentProvider = {
      ...
      delay: 0,
      callApi: vi.fn(),
      runSimba: vi.fn().mockImplementation(async function() {
        callOrder.push('simba');
        return runResults.map((result) => ({ ...result }));
      }),
    } as unknown as ApiProvider & { runSimba: Mock };
    ...
    expect(agentProvider.runSimba as Mock).toHaveBeenCalledTimes(1);
    expect(agentProvider.callApi as Mock).not.toHaveBeenCalled();
  ...
  it('evaluate with vars from file', async () => {
    ...
    vi.spyOn(fs, 'readFileSync').mockImplementation(function(path) {
    ...
    mockRenderPrompt.mockImplementation(async function(prompt, vars) {
  ...
  it('evaluate with _conversation variable', async () => {
    const mockApiProvider: ApiProvider = {
      id: vi.fn().mockReturnValue('test-provider'),
      callApi: vi.fn().mockImplementation(function(prompt) {
        return Promise.resolve({
          output: prompt,
          tokenUsage: { total: 10, prompt: 5, completion: 5, cached: 0, numRequests: 1 },
        });
  ...
  it('should maintain separate conversation histories based on metadata.conversationId', async () => {
    const mockApiProvider = {
      id: () => 'test-provider',
      callApi: vi.fn().mockImplementation(function(_prompt) {
        return ({
          output: 'Test output'
        });
      }),
  ...
  it('should handle evaluation timeout', async () => {
    ...
    const slowApiProvider: ApiProvider = {
      ...
      callApi: vi.fn().mockImplementation(function() {
  ...
  it('should abort when exceeding maxEvalTimeMs', async () => {
    ...
    const slowApiProvider: ApiProvider = {
      id: vi.fn().mockReturnValue('slow-provider'),
      callApi: vi.fn().mockImplementation(function(_, __, opts) {
  ...
  it('forces cache busting for repeat iterations', async () => {
    ...
    const provider: ApiProvider = {
      ...
      callApi: vi
        .fn()
        .mockImplementation(async function(_prompt: string, context?: Record<string, any>) {
  ...
  it('should include sessionId in metadata for afterEach hook', async () => {
    ...
    const mockedRunExtensionHook = vi.mocked(runExtensionHook);
    mockedRunExtensionHook.mockImplementation(async function(_extensions, hookName, context) {
